...
(async (): Promise<void> => {
  ...
  let endpoint =
    'https://api.x.immutable.com/v1/orders?include_fees=true&status=active&sell_token_address=0x9e0d99b864e1ac12565125c5a82b59adea5a09cd&direction=asc&buy_token_type=ETH&order_by=buy_quantity&direction=asc&page_size=2';
  ...
  await axios
    ...
    .then((response) => {
      ...
      if (priceFloor < halfPriceFloorSecond) {
        ...
        axios
          .post('https://api.x.immutable.com/v1/signable-order-details', data)
          .then((response) => {
